...
@interface MFormQuestionView ()

@end

@implementation MFormQuestionView
...
-(void) viewLoad
{
    ...
    for (Model_Item* item in qItem) {
        ...
        for (Model_Item* selectItem in item.aItems) {
            ...
            if(count==1)
            {
                UILabel* labTitle = [[UILabel alloc] initWithFrame:CGRectMake(_w, _height,textsize.width , textsize.height)];
            ...
            rbtn.frame = CGRectMake(_w-15+textsize.width/2, _height+45, 30, 30);
...
-(IBAction)checked:(id)sender
{
    ...
    aitem.ItemAnswer.S=btn.selected==YES?@"false":@"true";
    [self viewLoad];
}
